...
// Test 27: Generated JavaScript runs with runtime
test("generated JavaScript runs with runtime", () => {
  const tmp = `/tmp/integration_run_${Date.now()}.tsl`;
  writeFileSync(tmp, 'print("integration")\n', 'utf-8');
  try {
    const result = execSync(`node ${CLI} "${tmp}"`, { encoding: 'utf-8' });
    const jsMatch = result.match(/--- Generated JavaScript ---\s*\n([\s\S]*?)\s*--- End of Generated Code ---/);
    const jsCode = jsMatch[1];

    // Write a test runner that includes runtime
    const runner = `/tmp/integration_run_${Date.now()}.js`;
    const runtimeCode = readFileSync(RUNTIME, 'utf-8');
    const { print, range, clear, draw_rect, draw_circle, draw_line } = require(RUNTIME);
    
    // Execute with runtime context
    const fn = new Function('print', 'range', 'clear', 'draw_rect', 'draw_circle', 'draw_line', jsCode);
    fn(print, range, clear, draw_rect, draw_circle, draw_line);
  } finally {
